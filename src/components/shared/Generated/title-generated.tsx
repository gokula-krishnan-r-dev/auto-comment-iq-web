...
const TitleGeneraedWithAI = ({
  ...
  const isTitle = aiReply?.length === 0;
  ...
  const handleFormSubmit = (event: any) => {
    ...
    // Call your AI function to generate a reply based on inputValue
    const aiResponse: any = generateAiReply(inputValue);
    ...
    <div className="flex  flex-col pb-6">
      <ScrollArea className="h-[80vh]">
        <div className="h-full flex flex-col justify-center">
          <div className=" max-w-5xl w-full text-center mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <h1 className="text-base pb-8 font-bold text-gray-800 sm:text-4xl dark:text-white">
              Hi GOKULA KRISHNAN 👋
            </h1>
          ...
          {isTitle ? (
            <div className="mt-10 max-w-2xl w-full mx-auto px-4 sm:px-6 lg:px-8">
              <div className="relative">
                <form onSubmit={handleFormSubmit}>
                  <input
                    type="text"
                    className="p-4 block w-full border-gray-200 rounded-full text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-gray-400 dark:focus:ring-gray-600"
            ...
            <div
              className={cn("mt-4 flex items-center justify-center flex-col")}
            >
              <div className="flex items-center pb-4 gap-2">
                <CircleButton onClick={refetchAiReply} className="">
                  Add More 5 Titles
                </CircleButton>
                {isAllCopy && (
                  <CircleButton
                    onClick={() => {
                      const allTitles: any = aiReply
                        .map((res: any) => res?.title)
                        .join("\n");
                      copyToClipboard(allTitles, 1);
                    }}
              ...
              <div className={cn("max-w-3xl", className)}>
                {aiReply?.map((res: any, index: number) => (
                  <div
                    key={index}
                    className="bg-white min-w-full rounded-3xl p-4 mb-4 flex items-center justify-between"
                  >
                    <span title={res?.title} className="text-xl font-bold">
                      {res?.title.startsWith('"') && res?.title.endsWith('"')
                        ? res?.title.slice(1, -1)
                        : res?.title}
                    </span>
                    <button
                      title={res?.title}
                      className="bg-blue-500 hover:bg-white duration-300 hover:text-black text-white px-4 py-2 rounded-full focus:outline-none"
                      onClick={() => copyToClipboard(res?.title, index)}
                    >
      ...
      <footer className="mt-auto max-w-4xl text-center mx-auto px-4 sm:px-6 lg:px-8">
        <p className="text-xs text-gray-600 dark:text-gray-500">
          © 2024 website_name. A product of
          <a
...
export default TitleGeneraedWithAI;
